...
export default function PostHarvestBuyRequests() {
    ...
    const fetchPayments = async () => {
      try {
        ...
        const filteredData = (
          ...
          (item) => item.type === "postHarvest" && item.status === "pending"
        );
  ...
  //   }
  // };
  const updateStatus = async (tokenId, newStatus, id) => {
    let statusToUpdate;
  ...
  // Calculate summary data for post-harvest only
  const totalRequests = filteredData.length;
  const totalRequested = filteredData.reduce(
                  ...
                  <tr>
                    <td colSpan="9" className="text-center p-8 text-gray-500">
                      <div className="flex flex-col items-center">
                        ...
                        No post-harvest requests found
                      </div>
                    </td>
                  </tr>
                )}
