...
//       setLoading(true);
//       // ✅ API ko call karo
//       const res = await fetch(`/api/farmer/postHarvest?userId=${userId}`, {
//         cache: "no-store",
//       });
...
export default function PostHarvest() {
  ...
  const filteredInvestments = data.filter((investment) => {
    const matchesSearch =
      investment.fullName.toLowerCase().includes(searchTerm.toLowerCase()) ||
      investment.primaryCommodity
        .toLowerCase()
        .includes(searchTerm.toLowerCase());

    const matchesStatus =
      statusFilter === "all" ||
      (investment.status || "pending") === statusFilter;
    ...
    const matchesDate =
      ...
      (dateFilter === "recent" &&
        new Date(investment.createdAt) >
          new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)) ||
      (dateFilter === "upcoming" &&
        new Date(investment.harvestDate) > new Date());
  ...
  return (
      <div className="p-2 text-black">
        ...
        {/* Filter Section */}
        <div className="bg-white p-4 rounded-lg shadow border border-[#6f9d7e] mb-4">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            {/* Search Input */}
            <div className="w-full flex flex-col">
              <label className="text-sm text-[#6F9D7E] mb-1">Search</label>
              <input
                type="text"
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                placeholder="Search farmer name or commodity..."
                className="input input-success bg-white w-full"
              />
            </div>
          ...
          {filteredInvestments.map((investment) => (
            ...
            >
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  <span className="text-gray-200 font-semibold">Yield:</span>
                  <span className="text-[#FFE990]">
                    {investment.totalYield} {investment.yieldUnit}
                  </span>
                </div>

                <div className="flex justify-between">
                  <span className="text-gray-200 font-semibold">Acreage:</span>
                  <span className="text-[#FFE990]">
                    {investment.totalAcres} acres
                  </span>
                </div>
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-[#FFE990]">
                    ${investment.totalInvestmentRequired.toLocaleString()}
                  </span>
                </div>
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-[#FFE990]">
                    {new Date(investment.harvestDate).toLocaleDateString()}
                  </span>
                </div>
              </div>
